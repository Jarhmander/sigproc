...
define COMPILE_C_CMDS
	@mkdir -p $(dir $@)
	$(strip ${CC} -o $@ -c ${CREATE_DEP} ${CFLAGS} ${SRC_CFLAGS} ${INCDIRS} \
...
define COMPILE_CXX_CMDS
	@mkdir -p $(dir $@)
	$(strip ${CXX} -o $@ -c ${CREATE_DEP} ${CXXFLAGS} ${SRC_CXXFLAGS} ${INCDIRS} \
	    ${SRC_INCDIRS} ${SRC_DEFS} ${DEFS} $<)
	@cp ${@:%$(suffix $@)=%.d} ${@:%$(suffix $@)=%.P}; \
	 sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	     -e '/^$$/ d' -e 's/$$/ :/' < ${@:%$(suffix $@)=%.d} \
...
define COMPILE_ASM_CMDS
	@mkdir -p $(dir $@)
	$(strip ${AS} -o $@ -c ${CREATE_DEP} ${ASFLAGS} ${SRC_ASFLAGS} ${INCDIRS} \
	    ${SRC_INCDIRS} ${SRC_DEFS} ${DEFS} $<)
	@cp ${@:%$(suffix $@)=%.d} ${@:%$(suffix $@)=%.P}; \
	 sed -e 's/#.*//' -e 's/^[^:]*: *//' -e 's/ *\\$$//' \
	     -e '/^$$/ d' -e 's/$$/ :/' < ${@:%$(suffix $@)=%.d} \
...
define INCLUDE_SUBMAKEFILE
    ...
    ifneq "$$(strip $${SOURCES})" ""
        ...
        $${OBJS}: SRC_ASFLAG   := $${$${TGT}_ASFLAGS} $${SRC_ASFLAGS}
